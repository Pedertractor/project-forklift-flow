...
| **Pallet destinado à máquina bloqueia abastecimento**       | Enquanto houver `DeliveryTask` em aberto para a máquina (recebimento, preparo ou a caminho), o operador **só pode pedir retirada** — não abastecimento nem retirada+abastecimento, até a entrega na máquina (`COMPLETED`) |
