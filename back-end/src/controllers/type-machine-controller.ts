...
async function readMultipartTypeMachineFields(request: FastifyRequest): Promise<{
  name?: string
  urlImage?: string
  imageFile?: MultipartFile
}> {
  const out: { name?: string; urlImage?: string; imageFile?: MultipartFile } = {}
...
export const postCreateTypeMachine: RouteHandlerMethod = async (request, reply) => {
  if (request.isMultipart()) {
    const { name: rawName, urlImage: rawUrl, imageFile } =
      await readMultipartTypeMachineFields(request)
    ...
    if (imageFile) {
      try {
        urlImage = await saveTypeMachineImageFile(UPLOAD_ROOT_ABSOLUTE, imageFile)
      } catch (error) {
      ...
      if (u === '') {
        return reply
          .status(400)
          .send({ error: 'Envie o arquivo image (campo image) ou urlImage (texto).' })
      }
      urlImage = u
    }
  ...
  if (typeof urlImage !== 'string' || urlImage.trim() === '') {
    return reply.status(400).send({ error: 'Informe urlImage (texto nao vazio).' })
  }
...
export const getListTypeMachines: RouteHandlerMethod = async (_request, reply) => {
  const types = await listTypeMachines()
  return reply.send({ typeMachines: types })
}

export const getTypeMachineByIdHandler: RouteHandlerMethod = async (request, reply) => {
...
export const patchUpdateTypeMachine: RouteHandlerMethod = async (request, reply) => {
  const { typeMachineId } = request.params as { typeMachineId?: string }
  if (!typeMachineId) {
    return reply.status(400).send({ error: 'typeMachineId invalido.' })
  }
...
export const deleteTypeMachineHandler: RouteHandlerMethod = async (request, reply) => {
  const { typeMachineId } = request.params as { typeMachineId?: string }
  if (!typeMachineId) {
    return reply.status(400).send({ error: 'typeMachineId invalido.' })
  }
