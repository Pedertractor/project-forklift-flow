...
- **Pedido combinado** (retirada + abastecimento na mesma solicitação): só cria par genuinamente novo, vínculo criado na mesma transação (`requestPickupWithReplenishment`). **Bloqueado** (`OperatorSupplyRequestAlreadyOpenError`, 409) se já houver aviso elegível ainda não reivindicado na máquina (`findFirstEligibleUnclaimedForMachine`) — nesse caso o operador deve pedir apenas a retirada avulsa (`/pickup-only`), que amarra automaticamente no aviso já aberto. Evita dois caminhos de UI fazendo a mesma coisa (um explícito, um implícito).
...
| `POST` | `/pickup-with-replenishment` | Cria `PickupTask` + `OperatorMachineSupplyRequest` OPEN juntos, já amarrados na mesma transação (par genuinamente novo, sem corrida possível). Body opcional `{ isCritical?: boolean }`. **Bloqueado** (409, `OperatorSupplyRequestAlreadyOpenError`) se já houver aviso elegível ainda não reivindicado na máquina — peça só `/pickup-only`, que amarra automaticamente. **Bloqueado** (409, `OperatorRequestBlockedByPalletAtReceivingError`) enquanto houver `DeliveryTask` em aberto para a máquina (até entrega / `COMPLETED`) |
| `POST` | `/supply-only` | Aviso ao abastecimento; no máximo um `OPEN` por máquina — se já houver um `OPEN`, retorna o existente sem criar 2º (`created: false`). Se já houver retirada avulsa aberta e sem vínculo na máquina, o aviso novo amarra nela (`linkNewSupplyRequestToEligiblePickup`). **Bloqueado** (409, `OperatorRequestBlockedByPalletAtReceivingError`) enquanto houver `DeliveryTask` em aberto para a máquina (neste caso o operador só pode `/pickup-only` até o pallet ser entregue) |
...
5. Operador **retirada + abastecimento** (pedido combinado) → `PickupTask` + aviso amarrados na criação (mesma transação); sugestão de viagem para o transporte **após** abastecimento marcar o pallet pronto (`preparedAt`). Só é permitido quando a máquina **não** tem `DeliveryTask` em aberto nem aviso de abastecimento já `OPEN`/elegível (itens 9 e 11) — nesses casos o operador pede apenas retirada (item 3), que amarra sozinha.
...
11. Com um aviso de abastecimento já `OPEN` (ou elegível/não reivindicado) para a máquina, o operador **não** pode solicitar um novo abastecimento (`/supply-only` retorna o existente sem criar 2º) nem retirada+abastecimento (`/pickup-with-replenishment` é **bloqueado**, 409) — apenas retirada avulsa (`/pickup-only`, item 3), que amarra automaticamente no aviso já aberto. Evita um 2º card "Entrega + Retirada" duplicado quando o operador pede mais de uma retirada em sequência após um único abastecimento.
