...
export function SupplyPendingPreparationPageView(vm: SupplyPendingPreparationPageViewModel) {
      ...
      <div className="mx-auto w-full max-w-6xl">
        <header className="mb-6 border-b border-zinc-200 pb-6">
          ...
          <p className="mt-1.5 text-sm text-zinc-600">
            Pedidos em «Aguardando preparo» após a dobra finalizar sem pallet pronto. Ao concluir o
            preparo físico, marque <strong>pallet pronto</strong> para liberar na fila do transporte.
          </p>
        </header>
        ...
        {!hasSector ? (
          <p className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-950">
            Seu usuário não tem <strong>setor</strong> vinculado. A API exige setor para listar
            preparos pendentes do chão. Solicite ao administrador o ajuste do cadastro.
          </p>
        ) : null}
        ...
        {hasSector && user?.sector ? (
          <p className="mt-2 text-sm text-zinc-600">
            Setor:{' '}
            <span className="font-medium text-zinc-900">{user.sector.typeSector}</span>
          </p>
        ) : null}
          ...
          <table className="w-full min-w-[880px] border-collapse text-left text-sm">
            <thead>
              <tr className="border-b border-zinc-200 bg-zinc-50/90">
                <th className="px-4 py-3 font-semibold text-zinc-700">Máquina destino</th>
                ...
                rows.map((row) => (
                  <tr key={row.id} className="border-b border-zinc-100 last:border-0">
                    <td className="px-4 py-3">
                      ...
                      <div className="text-xs text-zinc-500">{row.destination.position}</div>
                    </td>
                    ...
                    <td className="px-4 py-3 text-right">
                      ...
                      >
                        {markMut.isPending ? 'Salvando…' : 'Marcar pallet pronto'}
                      </Button>
                    </td>
                  </tr>
                ))
        ...
        <div className="mt-6 rounded-xl border border-zinc-200 bg-zinc-50/80 px-4 py-3 text-sm text-zinc-700">
          <p className="m-0 text-xs font-semibold uppercase tracking-wide text-zinc-500">Fluxo</p>
          <p className="mt-2 mb-0 leading-relaxed">
            Use também a lista em «Solicitações de reposição» para abrir pedidos antecipados ou
            ajustar prioridade. Cubos marcados como pronto entram na fila compatível com o tipo
